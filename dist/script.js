...
window.addEventListener('DOMContentLoaded', function () {
  ...
  slider.render();
  var sliderPageModules = new _modules_slider_slider_main__WEBPACK_IMPORTED_MODULE_0__["default"]({
    container: '.moduleapp',
    btns: '.next'
  });
  sliderPageModules.render();
  var miniUpSlider = new _modules_slider_slider_mini__WEBPACK_IMPORTED_MODULE_2__["default"]({
    container: '.showup__content-slider',
    next: '.showup__next',
    prev: '.showup__prev',
    activeClass: 'card-active',
    //классы записываютс без .
    animate: true
  });
  miniUpSlider.init();
  ...
  new _modules_mask__WEBPACK_IMPORTED_MODULE_5__["default"]('[name="phone"]').init();
  ...
  _createClass(Difference, [{
    ...
    value: function bindTriggers(container, items, counter) {
      container.querySelector('.plus').addEventListener('click', function () {
        if (counter !== items.length - 2) {
          //если счетчик не равен последним 2 карточкам
    ...
    value: function hideItems(items) {
      items.forEach(function (item, i, arr) {
        // i - порядковый №, arr - ссылка на [] где items,т.е.oldOfficer или new Officer
        if (i !== arr.length - 1) {
          //если не явл.посл.массивом 
...
function () {
  ...
  _createClass(Download, [{
    ...
    value: function downloadItem(path) {
      ...
      var elem = document.createElement('a'); //'a' создается ссылка
...
function () {
  ...
  _createClass(Forms, [{
    ...
    value: function checkTextInputs() {
      //заполнение email только на латинице 
      this.txtInputs = document.querySelectorAll('[name="email"]');
      this.txtInputs.forEach(function (input) {
        ...
        input.addEventListener('input', function () {
          //если польз.внес сохраненные данные на кирилл, то они очищаются
    ...
    value: function postDatas() {
      ...
      this.forms.forEach(function (item) {
        item.addEventListener('submit', function (e) {
          ...
          postData('assets/question.php', formData).then(function (res) {
            console.log(res);
  ...
  _createClass(VideoPlayer, [{
    ...
    value: function bindTriggers() {
      ...
      this.btns.forEach(function (btn, i) {
        //i - номер 
        try {
          var blockedElem = btn.closest('.module__video-item').nextElementSibling; //получаем блок родителя сл. эл.

          if (i % 2 == 0) {
            //i % 2 определение четности/нечетности, 0 - четность
        ...
        btn.addEventListener('click', function () {
          if (!btn.closest('.module__video-item') || btn.closest('.module__video-item').getAttribute('data-disabled') !== 'true') {
            _this.activeBnt = btn; //получаем доступ к кн., ктр кликнули, чтобы отследить какая за ней

            if (document.querySelector('iframe#frame')) {
              //если плеер уже создан (если не прописать это усл., то плеер будет созд. каждый раз при наж.btn)
              _this.overlay.style.display = 'flex'; //то показываем мод окно с ним    

              if (_this.path !== btn.getAttribute('data-url')) {
                //если путь видео не равен этому аттрибуту,
                _this.path = btn.getAttribute('data-url'); //то создается этот путь

                _this.player.loadVideoById({
                  videoId: _this.path
                }); //и загружается видео из youtube по этой ссылке

              }
            } else {
              //если нет, то создаем     
              _this.path = btn.getAttribute('data-url'); //data-url указана ссылка на видео

              _this.createPlayer(_this.path); //при нажатии на кнопку создается плеер с ссылкой на видео  
    ...
    value: function bindCloseBtn() {
      ...
      this.close.addEventListener('click', function () {
        ...
        _this2.player.stopVideo(); //в плеере завершаем видео
    ...
    value: function createPlayer(url) {
      this.player = new YT.Player('frame', {
        //блок скопирован из YouTube Player API, но внесены изменения
        height: '100%',
        width: '100%',
        videoId: "".concat(url),
        //будет динамически подк. из аргумента этого метода
        events: {
    ...
    value: function onPlayerStateChange(state) {
      try {
        var blockedElem = this.activeBnt.closest('.module__video-item').nextElementSibling; //получаем 1-го родителя сл. эл-та у активной кнопки

        var playBtn = this.activeBnt.querySelector('svg').cloneNode(true); //клонируем полностью селекор 'svg'(иконка play) у активной кн.

        if (state.data === 0) {
          //если польз. досмотрел видео до конца ("state.data === 0" прописано в документации)
          if (blockedElem.querySelector('.play__circle').classList.contains('closed')) {
            //если у забл.эл. есть этот класс
    ...
    value: function init() {
      if (this.btns.length > 0) {
        //если есть хоть одна кнопка, то условие сработает 
        var tag = document.createElement('script'); //этот блок кода скопирован из YouTube Player API 
...
function (_Slider) {
  ...
  _createClass(MainSlider, [{
    ...
    value: function showSlides(n) {
      ...
      //n = тек. нумерация слайда
      try {
        if (n > this.slides.length) {
          //если тек. индекс больше  кол-во слайдов, 
      ...
      try {
        ...
        if (n === 3) {
          //если на стр.3, то через 3 сек. появляется блок
    ...
    key: "toggleButtons",
    value: function toggleButtons(btnsmodule, n) {
      var _this3 = this;

      btnsmodule.forEach(function (btn) {
        btn.addEventListener('click', function (e) {
    ...
    value: function plusSlides(n) {
      //переключение слайдов,  n - будем передавать 1 или -1(вперед или назад)
    ...
    value: function render() {
      ...
      //главный метод, ктр будет выполнять главное действие на стр
      if (this.container) {
        try {
        ...
        this.btns.forEach(function (btn) {
          btn.addEventListener('click', function () {
            ...
            _this4.slides[_this4.slideIndex - 1].classList.add('slideInLeft');
          ...
          btn.parentNode.previousElementSibling.addEventListener('click', function (e) {
            //при клике на родителя предыдущего эл.
            e.preventDefault(); //отменяем действие ссылки

            _this4.slideIndex = 1;

            _this4.showSlides(_this4.slideIndex); //показ текущего слайда
...
function (_Slider) {
  ...
  function MiniSlider(container, next, prev, activeClass, animate, autoplay) {
    ...
    return _possibleConstructorReturn(this, _getPrototypeOf(MiniSlider).call(this, container, next, prev, activeClass, animate, autoplay)); //получаем св-ва этих объектов от родителя  
  }
  ...
  _createClass(MiniSlider, [{
    ...
    value: function decorizeSlides() {
      ...
      this.slides.forEach(function (slide) {
        slide.classList.remove(_this.activeClass); //все слайды становятся неактивными 

        if (_this.animate) {
          //если в параметрах прописано animate
      ...
      if (this.animate) {
        //если в параметрах прописано animate = true
    ...
    value: function nextSlide() {
      if (this.slides[1].tagName == 'BUTTON' && this.slides[2].tagName == 'BUTTON') {
        //если 2-ая и 3-ая слайды явл. кнопками  
      ...
      this.prev.addEventListener('click', function () {
        for (var i = _this2.slides.length - 1; i > 0; i--) {
          ...
          if (_this2.slides[i].tagName !== 'BUTTON') {
            //если посл.слайд не будет кн. 
            var active = _this2.slides[i];

            _this2.container.insertBefore(active, _this2.slides[0]); //то последний слайд становится первым
    ...
    value: function activateAnimation() {
      ...
      }, 5000); //запускается nextSlide() каждые 5 сек.

      this.container.addEventListener('mouseenter', function () {
        //мышка над слайдом, то автопереключение ост.
        clearInterval(paused);
      });
      this.container.addEventListener('mouseleave', function () {
        //мышку убрали,  setInterval запускается 
    ...
    value: function init() {
      try {
        ...
        if (this.autoplay) {
          //если указан этот параметр, то запускается setInterval
...
var Slider = function Slider() {
  ...
  } catch (error) {} //получаем всех детей контейнера
